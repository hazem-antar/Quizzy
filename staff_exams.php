...
session_start();
  ...
  $id = $_SESSION['user_id'];
	 ...
	 if (isset($_POST['exam']) && isset($_POST['day'])){
		$exam_id = $_POST['exam'];
    $date = $_POST['day'];
		$sql="Delete FROM quizzes WHERE quiz_id = '$exam_id' and date ='$date'" ;
		$con_examination->query($sql);
	}

  $query_exams = "SELECT quizzes.quiz_id ,courses.course_name, courses.course_code, quizzes.date,
	 			quizzes.start_time, quizzes.end_time, quizzes.questions_count, courses.students_count
  ...
  if($result_exams && mysqli_num_rows($result_exams) > 0){
    ...
    $exam_col_arr = array('quiz_id','course_name','course_code','date','start_time','end_time','questions_count','students_count');
...
<html>
  <head>
    ...
    <ul>
      ...
      <li><a id="active" href="staff_main.php">Exams</a></li>
  ...
  <body>
    <div id="white_box_1">
      ...
      <tr>
        ...
        <td><b>End Time</b></td>
        <td><b>N.O Questions</b></td>
        <td><b>N.O Students</b></td>
      </tr>
    </table><br><br>
    <input class= "button" onclick="location.href = 'create_exam.php'" value="New Exam">
    </div>
		...
		<script>
			function deleteRow(obj) {
				...
				var id = (document.getElementById("exams_table").rows[index].cells)[1].innerHTML;
        var date = (document.getElementById("exams_table").rows[index].cells)[4].innerHTML;

			$.ajax({
					type: "POST",
					url: 'staff_exams.php',
					dataType: "text",
					data: {exam:id , day:date}
					}).done(function(data) {
						table.deleteRow(index);
						alert('Exam successfully removed.');
						window.location.href = "staff_exams.php";
					});
			}
			</script>
		<script>
